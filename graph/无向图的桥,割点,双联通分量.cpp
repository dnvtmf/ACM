...
点双连通分支: 建立一个栈, 存储当前双连通分支, 在搜索图时, 每找到一条树枝边或后向边(非横叉边), 就把这条边加入栈中. 如果遇到某时满足dfn[u] $leq$ low[v], 说明u是一个割点, 同时把边从栈顶一个个取出, 直到遇到了边(u,v), 取出的这些边与其关联的点, 组成一个点双连通分支. 割点可以属于多个点双连通分支, 其余点和每条边只属于且属于一个点双连通分支.
...
int dfs[MAXV], low[MAXV];
int stk[MAXV], top;//栈 -DCC

//割点 点双连通分量
int cp[MAXV];//记录割点
int id[MAXE], cnt_dcc;//连通分量编号及总数 -DCC
void tarjan(int u, int order, int root, int pree) //重边对应边的id，否则对应父亲结点fa
{
    dfs[u] = low[u] = order++;
    int num = 0;
    for(int i = head[u]; i != -1; i = e[i].next)
    {
        ...
        if(!dfs[v])
        {
            num++;
            stk[top++] = i;//-DCC
            tarjan(v, order, root, i);
            if(low[u] > low[v])
                low[u] = low[v];
            if(u == root && num >= 2)cp[u] = 1;
            else if(u != root && dfs[u] <= low[v])
            {
                cp[u] = 1;//是割点
                //-DCC
                cnt_dcc++;
