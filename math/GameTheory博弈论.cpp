...
定义: 游戏状态为x, $sg(x)$ 表示状态x的sg函数值, 即$ sg(x) = \min{\{n|n \in N \cap n \notin F(x)\}} $, F(x)表示x能够达到的所有状态. 一个状态为必败态则 $sg(x)=0$.
...
topcoder srm 676 div1 B

Wizards and Numbers(Codeforces #144 div1 C)
	有x个的石子, 每次取走$a^k (k \geq 0)$个, 取走最后一个石子的人输.
	如果$x \% (a + 1)$为奇数, 先手输, 否则, 先手赢.
*/
